...
3. **Select the reviewer model and dispatch.** Behavior depends on harness:
   ...
   **Cross-family via OpenRouter (model diversity — catches same-family blind spots):** When the value you want is a *genuinely different model family* (not just a separate context window), run the OpenRouter reviewer. The Opus sub-agent shares Claude's training priors; an OpenRouter reviewer from openai / deepseek / qwen / x-ai does not. Trade-off: the script is a stateless API call — it only sees what you pass it, so **inline the relevant foundational context into the plan text** (it can't read docs itself).
   ...
   - Single diverse reviewer: `python3 ~/scripts/integrations/second-opinion-or.py --author-family anthropic --plan /tmp/so-plan.md --session <N> --workspace <ws>` (`--author-family` is whoever wrote the plan — `anthropic` when it's me; the script excludes that family and lets `openrouter/auto` pick the best reviewer from the rest, so no model slug is pinned. Always pass `--session`/`--workspace` so the usage+cost log line is attributable).
   ...
   - Cost lever (cc#241): the script defaults `cost_quality_tradeoff: 3` ("favor quality over cost" — a strong reviewer without the single priciest frontier model every call). For a high-stakes review where you want the most capable model regardless of cost, add `--high-quality` (= tradeoff 0, the pre-cc#241 behavior). Tune anywhere 0–10 via `--cost-quality-tradeoff N` (10 = cheapest).
